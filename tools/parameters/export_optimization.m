...
if do_save
    ...
    export_path = char(fullfile('params', nlp.Name, strcat(gaitName, '_', label_append)));
    ...
    export_name = fullfile(export_path, strcat(param_name, '_', label_append, '.yaml'));
    yaml_write_file(export_name, export_params);
    
    % Export peripheral data
    info_path = fullfile(export_path, 'info');
    if ~exist(info_path, 'dir')
        mkdir(char(info_path));
    end
    
    constr_out = fullfile(info_path, 'constr.txt');
    varibs_out = fullfile(info_path, 'vars.txt');
    costs_out  = fullfile(info_path, 'cost.txt');
