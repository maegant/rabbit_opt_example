...
model_bounds.step_length   = [0.3, 0.5]; %essential: how long should a step last
...
bounds.(vNames{1}).inputs.ConstraintWrench.fRightSole.lb = -1000;
bounds.(vNames{1}).inputs.ConstraintWrench.fRightSole.ub =  1000; 
...
bounds.(vNames{1}).inputs.Control.u.lb = -100 * ones(4,1);
bounds.(vNames{1}).inputs.Control.u.ub = 100 * ones(4,1);
bounds.(vNames{1}).inputs.Control.u.x0 = zeros(4,1);
